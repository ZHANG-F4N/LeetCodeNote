...
所以，该方法适用于**区间频繁修改**，而且这个**区间范围是比较大**的，**离线查询**的情况。
...
可以看到，如果需要对 L~R 范围内所有数都进行相同的操作，我们不需要从L~R遍历arr然后在每个值上进行相同操作，只需要在差分数组d中改变L和R+1的值即可。但是在查询arr数组中某个位置的数时，却要根据差分数组从前往后递推求值。
